# Auto Novel Writer - Writing Guidance Builder
# Build writing guidance based on genre profile and reader signals

from __future__ import annotations

from typing import Any, Dict, List, Optional

from backend.services.genre_service import to_profile_key
...
GENRE_GUIDANCE_TEXT: dict[str, str] = {
    ...
    "history-travel": "题材加权：知识优势需有合理实现路径，避免过度开金手指。",
    "game-lit": "题材加权：游戏机制需清晰，数值变化可视化。",
    "sci-fi": "题材加权：科幻设定需服务于人物冲突，避免纯设定展示。",
}


# ============================================
# Methodology Anchors by Genre
# ============================================
...
def build_methodology_strategy_card(
    *,
    ...
    """Build a methodology strategy card for writing guidance."""
...
def build_methodology_guidance_items(strategy_card: Dict[str, Any]) -> List[str]:
    """Convert a strategy card into human-readable guidance items."""
...
def build_guidance_items(
    *,
    ...
    hook_diversify_enabled: bool = True,
) -> Dict[str, Any]:
    """Build comprehensive writing guidance items."""
    guidance: List[str] = []
...
def build_writing_checklist(
    *,
    ...
    """Build a writing checklist from guidance and signals."""
...
def is_checklist_item_completed(item: Dict[str, Any], reader_signal: Dict[str, Any]) -> bool:
    """Check if a checklist item is completed based on reader signals."""
    ...
    if source.startswith("methodology."):
        # Methodology items are soft prompts, not scored
        return True
...
# ============================================
# Complete Guidance Builder
# ============================================

class GuidanceBuilder:
    """Builder for comprehensive writing guidance."""

    def __init__(
        self,
        genre_profile: Dict[str, Any],
        reader_signal: Optional[Dict[str, Any]] = None,
    ):
        self.genre_profile = genre_profile
        self.reader_signal = reader_signal or {}
    ...
    def build_full_guidance(
        ...
        *,
        low_score_threshold: float = 75.0,
        hook_diversify_enabled: bool = True,
        min_checklist_items: int = 5,
        max_checklist_items: int = 12,
        default_weight: float = 1.0,
    ) -> Dict[str, Any]:
        """Build complete writing guidance package."""
        # Build strategy card
        strategy_card = build_methodology_strategy_card(
            chapter=chapter,
            reader_signal=self.reader_signal,
            genre_profile=self.genre_profile,
        )

        # Build guidance items
        guidance_result = build_guidance_items(
            chapter=chapter,
            reader_signal=self.reader_signal,
            genre_profile=self.genre_profile,
            low_score_threshold=low_score_threshold,
            hook_diversify_enabled=hook_diversify_enabled,
        )

        # Build methodology items
        methodology_items = build_methodology_guidance_items(strategy_card)

        # Build checklist
        checklist = build_writing_checklist(
            guidance_items=guidance_result["guidance"],
            reader_signal=self.reader_signal,
            genre_profile=self.genre_profile,
            strategy_card=strategy_card,
            min_items=min_checklist_items,
            max_items=max_checklist_items,
        ...
        completed_count = sum(
            ...
            if is_checklist_item_completed(item, self.reader_signal)
        )

        return {
            "chapter": chapter,
            "genre": self.genre_profile.get("genre", ""),
            "profile_key": self.genre_profile.get("profile_key", "general"),
            "strategy_card": strategy_card,
            ...
            "checklist_completion": {
                ...
                "total": len(checklist),
                "percentage": round(completed_count / max(len(checklist), 1) * 100, 2),
            },
            "risk_flags": strategy_card.get("signals", {}).get("risk_flags", []),
        }

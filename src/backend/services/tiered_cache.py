...
class TieredCache:
    ...
    def set(
        ...
        Args:
            ...
            tier: "l1", "l2", "l3", or "auto" (default: small->L1, large->L2)
        """
        if tier == "auto":
        ...
        if tier == "l1":
            ...
            if self.l2 is not None:
                ...
                # Fallback to L1 if L2 unavailable
                self.l1.set(key, value, ttl=ttl)
        elif tier == "l3":
    ...
    def invalidate_pattern(self, pattern: str) -> int:
        ...
        if self.l2 is not None:
            # diskcache does not support pattern delete natively;
            # iterate over keys.  This is slow for large caches.
            for k in list(self.l2):
                if pattern in k:
                    if self.l2.delete(k):
                        count += 1
        # L3 pattern delete via SQL LIKE

...
class SQLAlchemyPlotThreadRepository(PlotThreadRepositoryInterface):
    ...
    async def get_by_id(self, id: int) -> Optional[PlotThread]:
        result = await self.db.execute(
            select(PlotThread).where(PlotThread.id == id)
        )
    ...
    async def list(self, skip: int = 0, limit: int = 100, **filters) -> List[PlotThread]:
        stmt = select(PlotThread)
        for column, value in filters.items():
            if hasattr(PlotThread, column) and value is not None:
                stmt = stmt.where(getattr(PlotThread, column) == value)

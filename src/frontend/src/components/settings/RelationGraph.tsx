...
const ENTITY_TYPE_CONFIG: Record<EntityNodeType, { label: string; color: string; icon: typeof Users; glowColor: string; size: number }> = {
  character: { label: '角色', color: '#e8b87d', icon: Users, glowColor: 'rgba(232, 184, 125, 0.5)', size: 8 },
  item: { label: '物品', color: '#9b7ed9', icon: Scroll, glowColor: 'rgba(155, 126, 217, 0.5)', size: 6 },
  location: { label: '地点', color: '#5eb5a6', icon: MapPin, glowColor: 'rgba(94, 181, 166, 0.5)', size: 7 },
  faction: { label: '势力', color: '#d45d5d', icon: Swords, glowColor: 'rgba(212, 93, 93, 0.5)', size: 7 },
  world: { label: '世界观', color: '#5e6ad2', icon: Globe, glowColor: 'rgba(94, 106, 210, 0.5)', size: 6 },
  rule: { label: '规则', color: '#c8aa6e', icon: BookOpen, glowColor: 'rgba(200, 170, 110, 0.5)', size: 5 },
  outline: { label: '大纲', color: '#5b8ee8', icon: BookOpen, glowColor: 'rgba(91, 142, 232, 0.5)', size: 5 },
  ifline: { label: 'IF线', color: '#7eb84a', icon: Scroll, glowColor: 'rgba(126, 184, 74, 0.5)', size: 6 },
}
    ...
    <div className="h-full flex items-center justify-center bg-[var(--color-surface-base)] relative overflow-hidden">
      ...
      <div className="text-center relative z-10">
        <div className="relative mx-auto mb-4 w-10 h-10">
          ...
          <div className="absolute inset-1 rounded-full border-2 border-[var(--accent-primary)] border-b-transparent animate-spin" style={{ animationDirection: 'reverse', animationDuration: '1.5s' }} />
          <Sparkles className="w-4 h-4 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[var(--accent-primary)]" />
        </div>
        <p className="text-xs text-[var(--text-tertiary)] font-medium">加载图谱引擎...</p>
        <p className="text-[10px] text-[var(--text-disabled)] mt-1">正在构建节点关系</p>
      </div>
    </div>
  )
}
...
function NodeDetailPanel({ detail, onClose }: { detail: NodeDetail; onClose: () => void }) {
  ...
  return (
    <motion.div
      ...
      style={{
        left: Math.min(detail.x + 16, (typeof window !== 'undefined' ? window.innerWidth : 800) - 280),
        top: Math.max(detail.y - 16, 8),
        background: 'linear-gradient(145deg, rgba(22, 24, 28, 0.98), rgba(15, 16, 20, 0.98))',
    ...
    >
      ...
      <div className="flex items-start justify-between mb-2.5">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-semibold text-[var(--text-primary)]">{detail.node.name}</p>
            <p className="text-[10px] text-[var(--text-tertiary)] uppercase tracking-wider">{config.label}</p>
          </div>
        </div>
        <button
          onClick={onClose}
  ...
  return (
    <div className="absolute top-3 left-3 z-10 flex flex-col gap-2">
      ...
      <div className="flex flex-col gap-0.5 rounded-xl p-1.5"
        style={{
          background: 'linear-gradient(145deg, rgba(30, 32, 38, 0.95), rgba(22, 24, 28, 0.95))',
          border: '1px solid rgba(255, 255, 255, 0.08)',
          boxShadow: '0 4px 20px rgba(0,0,0,0.4), 0 0 0 1px rgba(255,255,255,0.02)',
        }}
      >
      ...
      >
        ...
        >
          ...
          <span className="text-[10px] text-[var(--text-tertiary)] group-hover:text-[var(--text-secondary)] transition-colors font-medium">筛选</span>
          <ChevronRight
          ...
          {isExpanded && (
            ...
            >
              ...
              <div className="pt-2 mt-2 border-t border-white/5">
                <p className="text-[10px] mb-1.5 px-2 text-[var(--text-tertiary)] font-medium">关系类型</p>
                <select
...
function FilterButton({ icon: Icon, title, onClick }: { icon: typeof ZoomIn; title: string; onClick: () => void }) {
  return (
    <button
...
function Legend({
  ...
  return (
    <motion.div
      ...
      style={{
        minWidth: '160px',
        background: 'linear-gradient(145deg, rgba(22, 24, 28, 0.97), rgba(15, 16, 20, 0.97))',
        border: '1px solid rgba(255, 255, 255, 0.08)',
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-white/5">
        <span className="text-[10px] font-semibold text-[rgba(255,255,255,0.5)] uppercase tracking-wider">图例</span>
        <button
          onClick={onToggle}
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-2 mb-3">
          {(Object.entries(ENTITY_TYPE_CONFIG) as [EntityNodeType, typeof ENTITY_TYPE_CONFIG['character']][]).map(([type, config]) => (
            <div key={type} className="flex items-center gap-2">
              ...
              />
              <span className="text-[10px] text-[var(--text-secondary)]">{config.label}</span>
            </div>
          ))}
        </div>
        ...
        {uniqueTypes.length > 0 && (
          <>
            <div className="border-t border-white/5 pt-2.5">
              <span className="text-[9px] text-[var(--text-disabled)] font-medium uppercase tracking-wider mb-2 block">关系类型</span>
                ...
                {uniqueTypes.slice(0, 6).map((type) => (
                  <div key={type} className="flex items-center gap-2.5">
                    <div
                      ...
                      style={{
                        backgroundColor: RELATION_TYPE_COLORS[type] || RELATION_TYPE_COLORS.other,
...
function StatsBar({
  ...
  return (
    <div className="absolute bottom-3 left-3 z-10 text-[10px] px-3 py-2 rounded-xl flex items-center gap-2.5"
      style={{
        background: 'linear-gradient(145deg, rgba(22, 24, 28, 0.95), rgba(15, 16, 20, 0.95))',
        border: '1px solid rgba(255, 255, 255, 0.06)',
        boxShadow: '0 4px 20px rgba(0,0,0,0.3), 0 0 0 1px rgba(255,255,255,0.02)',
        backdropFilter: 'blur(16px)',
      }}
    >
      <span className="text-[var(--text-secondary)] font-medium">{nodeCount} 节点</span>
      <span className="text-[var(--text-disabled)]">·</span>
      <span className="text-[var(--text-secondary)] font-medium">{linkCount} 关系</span>
      {filterRelation !== 'all' && (
        <>
          <span className="text-[var(--text-disabled)]">·</span>
          <button
...
function GraphBackground() {
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[500px] h-[250px] opacity-[0.04]"
        style={{
          background: 'radial-gradient(ellipse at center, var(--accent-100) 0%, transparent 70%)',
        }}
      />
    </div>
  )
  ...
  const nodeCanvasObject = useCallback((node: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
    ...
    if (isHovered || isSelected) {
      for (let i = 3; i > 0; i--) {
        ...
        glowGradient.addColorStop(0, `${config?.glowColor || 'rgba(94, 106, 210, 0.2)'}`)
    ...
    // Outer ring shadow
    ctx.shadowColor = isHovered ? (config?.color || '#5e6ad2') : 'rgba(0,0,0,0.4)'
    ...
    const nodeGradient = ctx.createRadialGradient(
      x - finalRadius * 0.35, y - finalRadius * 0.35, 0,
      x, y, finalRadius
    )
    ...
    const highlightGradient = ctx.createRadialGradient(
      x - finalRadius * 0.3, y - finalRadius * 0.3, 0,
      x, y, finalRadius * 0.7
    )
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 h-px z-10"
        style={{
          background: 'linear-gradient(90deg, transparent, rgba(94, 106, 210, 0.4), rgba(94, 106, 210, 0.15), rgba(94, 106, 210, 0.4), transparent)',
        }}
      />
      ...
      <Suspense fallback={<GraphFallback />}>
        {viewMode === '2d' ? (
          <ForceGraph2D
            ...
            nodeCanvasObject={renderMode === 'simple' ? undefined : nodeCanvasObject}
            linkCanvasObjectMode={() => 'replace'}
            linkCanvasObject={renderMode === 'simple' ? undefined : linkCanvasObject}
          />
        ) : (
          <ForceGraph3D
            ref={fgRef}
